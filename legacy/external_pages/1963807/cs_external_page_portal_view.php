...
.column_content .cc_button:hover,
...
.column_content .cc_button:active {
	position: relative;
	top: 1px;
    }

.column_content .cc_col_left h3,
.column_content .cc_col_right h3,
.column_content .cc_col_full h3 {
    padding: 10px;
    margin: 0px;
    font-size: 12px;
...
.column_content .cc_col_left p,
.column_content .cc_col_right p,
.column_content .cc_col_full p {
    padding: 10px;
    }
...
//if ( !$this->_environment->inPortal() || $this->_current_user->isGuest()) {
      ...
      if ( $this->_with_personal_area and empty($cs_mod)) {
         if ( !empty($this->_current_user) and ($this->_current_user->getUserID() == 'guest' and $this->_current_user->isGuest()) and !$this->_environment->inServer() ) {
            ...
            $html .= '<input type="submit" name="option" value="'.$this->_translator->getMessage('MYAREA_LOGIN_BUTTON').'" tabindex="4"/>'.LF;
            $html .= '</div>'.LF;
            #$html .= '</td></tr>'.LF;
            #$html .= '</table>'.LF;
            $html .= '</form>'.LF;
            if ( !$this->_environment->inServer() ) {
               ...
               if ( $count_auth_source_list_add_account != 0 ) {
                  ...
                  $activeLink = -1;
                  if (isset($serverNameExplode[0])) {
                    $serverNameMatches = array(
                      'schulintern',
                      'unterricht',
                      'institutionen'
                    );
                    
                    foreach ($serverNameMatches as $index => $match) {
                      if (mb_stristr($serverNameExplode[0], $match)) {
                        $activeLink = $match;
                        break;
                      }
                    }
                  ...
                  $html .= '<li><a style="display: inline;" href="//sh.schulcommsy.de/commsy/register/' . $activeLink . '/509947/1963807">Neue Kennung beantragen</a></li>'.LF;
#                  $html .= '<li>'.ahref_curl($this->_environment->getCurrentContextID(),$this->_environment->getCurrentModule(),$this->_environment->getCurrentFunction(),$params,$this->_translator->getMessage('MYAREA_LOGIN_ACCOUNT_WANT_LINK'),'','','','','','','style="display:inline;"').'</li>'.LF;
#                  $html .= '<span style="font-size:10pt;">&gt; '.ahref_curl($this->_environment->getCurrentContextID(),$this->_environment->getCurrentModule(),$this->_environment->getCurrentFunction(),$params,$this->_translator->getMessage('MYAREA_LOGIN_ACCOUNT_WANT_LINK'),'','','','','','','style="display:inline;"').'</span>'.BRLF;
            ...
            $html .= '<div style="float:right; text-align:right;"><a href="room/' . $this->_environment->getCurrentContextID() . '/logout" id="tm_logout" title="' . $translator->getMessage("LOGOUT") . '">'.$translator->getMessage("LOGOUT").'</a></div>'.LF;
 ...
 //} 
  ...
  function _getTableheadAsHTML($room_list_view) {
      ...
      if ($room_list_view->_environment->inPortal()) {
         ...
         if ( $room_list_view->getSortKey() == 'title' ) {
            $html .= ' <img src="css/external_portal_styles/'.$current_portal->getItemID().'/img/sort_up.gif" alt="&lt;" border="0"/>';
         } elseif ( $room_list_view->getSortKey() == 'title_rev' ) {
            $html .= ' <img src="css/external_portal_styles/'.$current_portal->getItemID().'/img/sort_down.gif" alt="&lt;" border="0"/>';
         }
         $html .= '      </td>'.LF;
         if ($room_list_view->_environment->inPortal()) {
...
#    $html .= '   </div>'.LF;
    ...
    $html .= '        &copy;' . date("Y") . ' SchulCommSy / effective webwork GmbH'.LF;
